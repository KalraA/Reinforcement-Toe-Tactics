...
class PolicyGradientREINFORCE(object):
  ...
  def create_variables(self):
    ...
    with tf.name_scope("compute_pg_gradients"):
      ...
      self.taken_actions = tf.placeholder(tf.float32, (None, 9), name="taken_actions")
      ...
      self.cross_entropy_loss = tf.nn.softmax_cross_entropy_with_logits(self.logprobs, self.taken_actions)
      self.pg_loss            = tf.reduce_mean(self.cross_entropy_loss)
      self.reg_loss           = sum([tf.reduce_sum(tf.square(x)) for x in policy_network_variables])
  ...
  def sampleAction(self, states, legal_moves = None):
